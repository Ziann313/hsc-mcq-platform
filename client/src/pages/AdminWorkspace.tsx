    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <section className="rounded-[24px] bg-white p-5 shadow-sm sm:p-6">
          ...
          {queue.isLoading ? <SkeletonRows /> : queue.data?.length ? <div className="mt-5 space-y-3">{queue.data.map(question => <article key={question.id} className="rounded-2xl border border-slate-100 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap gap-2"><Tag label="HUMAN REVIEW" /><Tag label={question.subject} muted /><Tag label={`v${question.version}`} muted /></div><p className="mt-3 line-clamp-2 text-sm font-bold leading-6 text-[#071d33]">{question.prompt}</p><p className="mt-1 text-xs text-slate-500">{copy("Difficulty", "কঠিনতা")}: {question.difficulty}</p></div><div className="flex flex-wrap gap-2"><ActionButton onClick={() => review.mutate({ questionId: question.id, status: "approved", note: "Approved from governance workspace" })} disabled={review.isPending} icon={Check} label={copy("Approve", "অনুমোদন")} /><ActionButton onClick={() => review.mutate({ questionId: question.id, status: "needs_review", note: "Flagged for follow-up" })} disabled={review.isPending} icon={Flag} label={copy("Flag", "ফ্ল্যাগ")} variant="amber" /><ActionButton onClick={() => review.mutate({ questionId: question.id, status: "archived", note: "Archived from governance workspace" })} disabled={review.isPending} icon={Archive} label={copy("Archive", "আর্কাইভ")} variant="muted" /></div></div></article>)}</div> : <EmptyQueue icon={FileText} title={copy("No questions are waiting for review", "কোনো প্রশ্ন রিভিউয়ের অপেক্ষায় নেই")} detail={copy("Questions appear here only after source mapping and intake validation.", "সোর্স ম্যাপিং ও ইনটেক ভ্যালিডেশনের পরই প্রশ্ন এখানে আসে।")} />}
        </section>
        <section className="rounded-[24px] bg-white p-5 shadow-sm sm:p-6">
