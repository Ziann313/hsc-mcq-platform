      ...
      <div className="relative grid gap-7 lg:grid-cols-[1fr_230px]">
        <div className="min-w-0">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            {isAuthenticated ? <Button onClick={() => navigate(hasPractice ? "/practice" : "/notices")} className="mcq-action-button h-11 rounded-xl bg-[#16b89b] text-[#071d33] hover:bg-[#24c8aa] sm:w-auto">{hasPractice ? <Play size={16} fill="currentColor" /> : <Bell size={16} />}{copy(hasPractice ? "Open approved practice" : "View official notices", hasPractice ? "অনুমোদিত প্র্যাকটিস খোলো" : "অফিসিয়াল নোটিশ দেখো")}</Button> : <Button onClick={() => startLogin()} className="mcq-action-button h-11 rounded-xl bg-[#16b89b] text-[#071d33] hover:bg-[#24c8aa] sm:w-auto"><LockKeyhole size={16} />{copy("Continue to secure sign-in", "নিরাপদ সাইন-ইন চালিয়ে যাও")}</Button>}
            <Button onClick={() => navigate("/practice")} variant="outline" className="mcq-action-button h-11 border-white/20 bg-white/5 text-white hover:bg-white/10 hover:text-white sm:w-auto"><BookOpenCheck size={16} />{copy("Practice availability", "প্র্যাকটিস অ্যাভেইলেবিলিটি")}</Button>
          </div>
        </div>
