    ...
    <div className="min-h-screen bg-[#f4f7f7] text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[264px] flex-col bg-[#071d33] px-4 py-5 text-slate-100 lg:flex">
        <button className="mb-8 flex items-center gap-3 px-2 text-left" onClick={() => navigate("/")}>
          ...
          <span><b className="block font-display text-xl tracking-tight">Shikha</b><span className="text-xs text-slate-400">Learn. Practice. Rise.</span></span>
        </button>
      ...
      <div className="lg:pl-[264px]">
        <header className="sticky top-0 z-30 flex h-[74px] items-center justify-between border-b border-slate-200/80 bg-[#f4f7f7]/85 px-4 backdrop-blur-lg sm:px-7 lg:px-10">
          <div className="flex items-center gap-3 lg:hidden">
            ...
            <span className="font-display text-xl font-bold text-[#071d33]">Shikha</span>
          </div>
      ...
      {mobileOpen && <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="Close menu" onClick={() => setMobileOpen(false)} className="absolute inset-0 bg-[#071d33]/50" /><aside className="relative flex h-full w-[82%] max-w-[330px] flex-col bg-[#071d33] px-4 py-5 text-slate-100 shadow-2xl"><div className="mb-7 flex items-center justify-between"><button onClick={() => navigate("/")} className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-2xl bg-[#16b89b]"><GraduationCap size={22} /></span><b className="font-display text-xl">Shikha</b></button><button onClick={() => setMobileOpen(false)} className="rounded-xl p-2 text-slate-300"><X /></button></div><nav className="space-y-1">{items.map(item => { const Icon = item.icon; return <button key={item.path} onClick={() => navigate(item.path)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left ${location === item.path ? "bg-[#16b89b] font-semibold text-[#04131f]" : "text-slate-200"}`}><Icon size={18} />{copy(item.label, item.bn)}</button>; })}</nav></aside></div>}
