...
export type FrozenAttemptQuestion = { questionId: number; correctOptionId: number; subject?: string; chapter?: string; marks?: number; negativeMarkWeight?: number };
...
export function buildExpiredAttemptFinalization(questions: FrozenAttemptQuestion[], selections: McqSelection[]) {
  ...
  const result = scoreMcqExam(questions.map(question => ({
    ...
    negativeMarkWeight: question.negativeMarkWeight ?? 0, subject: question.subject ?? "General", chapter: question.chapter ?? "General",
  })), selections);
